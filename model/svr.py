...
RANDOM_STATE = 42
OUT_PNG = "./outputs/model_comparison/svr/learning_curves.png"
OUT_CSV = "./outputs/model_comparison/svr/r2_results.csv"
...
def load_and_prepare(path=EXCEL_PATH):
    ...
    if df_num.shape[1] < 11:
        raise ValueError("At least 11 numeric columns are required.")
...
def main():
    ...
    if n_train_total < 18:
        raise ValueError("The training set is too small.")
    ...
    print(f"\n>>> Starting SVR training across {num_gradients} sample-size gradients with Bayesian optimization for {n_properties} properties...\n")
    ...
    for i in range(1, num_gradients + 1):
        ...
        for prop_idx in range(n_properties):
            ...
            optimizer = BayesianOptimization(
                f=svr_cv, pbounds=pbounds, random_state=RANDOM_STATE, verbose=0
            )

            optimizer.maximize(init_points=INIT_POINTS, n_iter=N_ITER)

            best_res = optimizer.max['params']
            best_C = 10 ** best_res['log_C']
            best_gamma = 10 ** best_res['log_gamma']
            best_epsilon = 10 ** best_res['log_epsilon']
            ...
            print(f"    - Best C: {best_C:.4g}, gamma: {best_gamma:.4g}, epsilon: {best_epsilon:.4g}")
            ...
            results["best_gamma"].append(best_gamma)
            results["best_epsilon"].append(best_epsilon)

        print()

    df_results = pd.DataFrame(results)
    df_results.to_csv(OUT_CSV, index=False)
    print(f">>> Completed. SVR results saved to {OUT_CSV}")
    ...
    plt.title("SVR Test $R^2$ Comparison Across Properties")
    plt.xlabel("Number of Training Samples")
    plt.ylabel("$R^2$ Score")
    plt.grid(True, alpha=0.3)
    plt.legend(loc="lower right")
    plt.tight_layout()
    plt.savefig("./outputs/model_comparison/svr/property_comparison.png", dpi=180)
    plt.show()
