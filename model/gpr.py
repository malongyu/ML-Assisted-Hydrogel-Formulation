...
def main():
    ...
    print(f"\n>>> Starting GPR training across {num_gradients} sample-size gradients with Bayesian optimization for {n_properties} properties...\n")
    ...
    for i in range(1, num_gradients + 1):
        ...
        for prop_idx in range(n_properties):
            ...
            print(
                f"    - Best alpha (noise): {best_alpha:.4g}, length scale: {best_length_scale:.4g}, constant: {best_constant:.4g}, nu: {best_nu}")
    ...
    print(f">>> Completed. GPR results saved to {OUT_CSV}")
